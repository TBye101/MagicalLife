...
namespace SoundConvert
{
    class Common
    {
        ...
        public const int NUM_COLUMNS = 50;
        public const int NUM_ROWS = 25;
        //        void (* Common_Private_Error) (FMOD_RESULT, string, int);

        //public void ERRCHECK_fn(MagicalLifeAPI.Sound.FMOD.RESULT result, string file, int line)
        //{
        //    if (result != MagicalLifeAPI.Sound.FMOD.RESULT.OK)
        //    {
...
string Common_MediaPath(string fileName)
{
    ...
    void Common_Format(string buffer, int bufferSize, string formatString...)
{
    va_list args;
...
void Common_Draw(const char* format, ...)
{
    ...
    do
    {
        ...
        if (copyLength > Common.NUM_COLUMNS)
        {
            // Hard wrap by default
            copyLength = Common.NUM_COLUMNS;

            // Loop for a soft wrap
            for (int i = Common.NUM_COLUMNS - 1; i >= 0; i--)
            {
                if (stringPtr[i] == ' ')
                {
                    copyLength = i + 1;
